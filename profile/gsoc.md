...
* Description: In SeBS, we support AWS Lambda, Google Cloud Functions, Azure Functions, and OpenWhisk. Last year, we continued adding new features to the benchmark: Knative and Fission platforms, ARM functions, and container support. However, clouds keep changing: new Azure Functions runtime versions, Google Cloud Functions version 2 ([issue 158](https://github.com/spcl/serverless-benchmarks/issues/158)), new AWS SDK for Node.js ([issue 207](https://github.com/spcl/serverless-benchmarks/issues/207)), new Amazon Linux version ([issue 208](https://github.com/spcl/serverless-benchmarks/issues/208)), and snapshot support on AWS ([issue 237](https://github.com/spcl/serverless-benchmarks/issues/237)). Furthermore, we need new features to keep the benchmark usable for the community: these can include help with resource cleaning to avoid leaving user's clouds full of SeBS artifacts once measurements are conducted ([issue 233](https://github.com/spcl/serverless-benchmarks/issues/233)), robust method of querying cloud logs and outputs ([issue 42](https://github.com/spcl/serverless-benchmarks/issues/42) and [issue 234](https://github.com/spcl/serverless-benchmarks/issues/234)), and better security policies ([issue 215](https://github.com/spcl/serverless-benchmarks/issues/215)). Finally, we should extensively test SeBS with serverless platforms deployed on Kubernetes ([issue 203](https://github.com/spcl/serverless-benchmarks/issues/203)) and using our deployment of object and NoSQL storage ([issue 202](https://github.com/spcl/serverless-benchmarks/issues/202)). Adding support for new runtimes and usability improvements would help SeBS stay usable and competitive in the changing world of clouds.
* Expected outcome: Contributing to SeBS support for new systems, runtimes, and usability features.
...
* Project size - 90 hours (small), 175 hours (medium) or 350 hours (large); depending on the scope and focus of the project.
* Difficulty - Medium/Hard (depending on the project's scope).
* Mentor - Marcin Copik (mcopik [at] gmail [.] com), [mcopik @ GitHub](https://github.com/mcopik/); Marcin Chrapek (marcin.chrapek [at] inf.ethz [.] ch)
...
* **Description**: In SeBS, we provide a representative set of functions, and we have developed a set of serverless workflows that will be included in the upcoming release. However, the serverless field is constantly changing, and new types of applications are being "FaaS-ified". Therefore, we would like to extend our benchmarking suite with new function candidates, such as utility functions (antivirus scanning) or scientific benchmarks, and new workflows. [An ongoing issue has a list of possible applications](https://github.com/spcl/serverless-benchmarks/issues/140).
* **Expected outcome**: Contributing to SeBS new benchmarking functions, triggers, and workloads that successfully execute on already supported platforms.
...
#### [PraaS, FMI] Serverless processes on functions
